...
export const solveStateMachine = {
    ...
    visitor: {
        ...
        AssignmentExpression(path) {
            ...
            const returnArg =
                body.length === 1 && body[0].isReturnStatement() ? body[0].get('argument') : null;
            ...
            console.log(
                ` |-> Found potential State Machine pattern with state var '${stateVarName}'`
            );
            ...
            if (setterName && calculatorName && logicMap.size > 0) {
                ...
                console.log(` |-> State Machine fully parsed!`);
            ...
            exit(path) {
                ...
                if (propName === info.setterName) {
                    ...
                    if (evaluation.confident) {
                        ...
                        console.warn(` |-> Could not determine state value for: ${gen(path.node)}`);
                    }
                }
                ...
                if (propName === info.calculatorName) {
                    ...
                    if (currentState === null) {
                        console.warn(
                            ` |-> Calculator called before state was set: ${gen(path.node)}`
                        );
                        return;
                    }
                    ...
                    if (!logicNode) {
                        console.warn(
                            ` |-> No logic for state ${currentState} in call: ${gen(path.node)}`
                        );
                        return;
                    }
