...
export const solveStateMachine = {
    ...
    visitor: {
        ...
        AssignmentExpression(path) {
            ...
            console.log(` |-> Found potential SM pattern with state var '${stateVarName}'`);
            ...
            exit(path) {
                ...
                if (propName === info.calculatorName) {
                  ...
                  if (currentState === null) {
                    console.warn(` |-> Calculator called before state was set: ${gen(path.node)}`);
                    return;
                  }

                  const logicNode = info.logicMap.get(currentState);
                  if (!logicNode) {
                    console.warn(` |-> No logic for state ${currentState} in call: ${gen(path.node)}`);
                    return;
                  }
