...
export const resolveGlobalObjectProxy = {
  priority: 100,
  visitor: {
    AssignmentExpression(path) {
      ...
      path.get('right.callee').traverse({
        CallExpression(callPath) {
          ...
          if (!callee.isMemberExpression() || !callee.get('object').isIdentifier({ name: 'Object' })) return;

          const prop = callee.get('property');
          const isDefineProperty = prop.isIdentifier({ name: 'defineProperty' }) || prop.isStringLiteral({ value: 'defineProperty' });
          if (!isDefineProperty) return;

          const descriptor = callPath.get('arguments.2');
          if (!descriptor || !descriptor.isObjectExpression()) return;
          
          const getProperty = descriptor.get('properties').find(p =>
            p.isObjectProperty() && (t.isIdentifier(p.node.key, { name: 'get' }) || t.isStringLiteral(p.node.key, { value: 'get' }))
          );
          if (!getProperty || !getProperty.get('value').isFunctionExpression()) return;

          const getterBody = getProperty.get('value.body.body');
          if (getterBody.length === 1 && getterBody[0].isReturnStatement() && getterBody[0].get('argument').isThisExpression()) {
            isGlobalGetter = true;
            callPath.stop();
          }
        }
      ...
      if (!programPath.node.injectedGlobal) {
        ...
        programPath.unshiftContainer('body', t.variableDeclaration('const', [
      ...
      for (let i = refs.length - 1; i >= 0; i--) {
          ...
          if (parentMemberExpr.isMemberExpression({ object: refPath.node }) &&
              t.isNodesEquivalent(parentMemberExpr.node.property, aliasMemberExpressionNode.property) &&
      ...
      console.log(` |-> Replaced all ${refs.length} references to '${proxyName}' and its alias.`);
      ...
      if (path.parentPath.isSequenceExpression()) {
        ...
        console.warn(` |-> Assignment found in an unexpected parent container: ${path.parentPath.type}. Cleanup skipped.`);
      }
