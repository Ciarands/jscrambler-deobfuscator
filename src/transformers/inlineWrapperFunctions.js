...
function isWrapperFunction(node, scope) {
  ...
  let typeofExpr = null;
  if (t.isUnaryExpression(test.left, { operator: 'typeof' }) && t.isStringLiteral(test.right, { value: 'function' })) {
    typeofExpr = test.left;
  } else if (t.isUnaryExpression(test.right, { operator: 'typeof' }) && t.isStringLiteral(test.left, { value: 'function' })) {
  ...
  if (!t.isCallExpression(consequent) || !t.isMemberExpression(consequent.callee) || !t.isIdentifier(consequent.callee.property, { name: 'apply' })) {
    return null;
  }
  if (!areNodesEquivalentAfterResolution(consequent.callee.object, targetExpression, functionBodyScope)) {
    return null;
  }
  const applyArgs = consequent.arguments;
  if (applyArgs.length !== 2) return null;
  if (!areNodesEquivalentAfterResolution(applyArgs[0], targetExpression.object, functionBodyScope)) {
      return null;
  }
    ...
    Program(programPath) {
      ...
      programPath.traverse({
        AssignmentExpression(path) {
            ...
            if (!areNodesEquivalentAfterResolution(baseObjectCandidate, baseObjectNode, path.scope)) return;
      ...
      console.log(` |-> Mapping complete. Found ${wrapperMap.size} unique wrapper definitions.`);
      ...
      programPath.traverse({
        CallExpression: {
          exit(callPath) {
            ...
            if (!areNodesEquivalentAfterResolution(callee.object, baseObjectNode, callPath.scope)) return;
        ...
        AssignmentExpression: {
            exit(assignmentPath) {
                ...
                if (!areNodesEquivalentAfterResolution(baseObjectCandidate, baseObjectNode, assignmentPath.scope)) return;
      ...
      programPath.traverse({
        SequenceExpression: {
          exit(p) {
            const filtered = p.node.expressions.filter(e => !t.isIdentifier(e, {name: "undefined"}));
